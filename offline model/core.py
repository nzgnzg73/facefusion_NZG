#!/usr/bin/env python3
"""
core_offline.py (Urdu commented)
Offline-safe full replacement for facefusion/core.py for FaceFusion v3.4.2.

نوٹ (اردو):
- یہ فائل آف لائن استعمال کے لیے تیار کی گئی ہے: کوئی نیٹ ورک ڈاؤنلوڈ یا curl کی ضرورت نہیں ہوگی۔
- ماڈلز اسی فولڈر سے لوڈ ہوں گے جو MODELS_PATH میں دیا گیا ہے۔
- EXECUTION_PROVIDERS میں 'cpu', 'cuda', یا 'openvino' رکھا جا سکتا ہے۔
"""
...
# نیٹ ورک سے بچنے کے لئے download helpers import نہ کریں
...
# ----------------- CONFIG (تبدیل کریں اگر ضروری ہو) -----------------
# آپ کے لوکل ماڈلز کا فولڈر۔ اسے اپنی مشین کے مطابق تبدیل کریں
MODELS_PATH = r'D:\Flie\facefusion-3.4.2\.assets\models'  # <-- یہاں ماڈل پاتھ تبدیل کریں

# Execution provider کا default سیٹ — 'cpu' یا 'cuda' یا 'openvino'
# مثال: EXECUTION_PROVIDERS = ['cuda'] اگر آپ GPU استعمال کر رہے ہیں
EXECUTION_PROVIDERS = ['cpu']  # <-- یہاں بدل سکتے ہیں

# ماڈل cache flag — مستقبل میں cache logic کا استعمال کریں اگر چاہیں
...
def pre_check() -> bool:
	...
	# offline: curl requirement disabled
...
def _find_local_model_by_filename(filename: str) -> Optional[str]:
	try:
		...
		if hasattr(module, 'create_static_model_set'):
			for model in module.create_static_model_set(state_manager.get_item('download_scope')).values():
				...
				if model_hash_set and model_source_set:
					# Instead of downloading, verify local presence
					for src in model_source_set:
						filename = os.path.basename(src)
						if filename and not _find_local_model_by_filename(filename):
							# missing model file locally
							return 1
	return 0
